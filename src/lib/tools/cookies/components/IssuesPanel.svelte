<script lang="ts">
	import type { CookieIssue } from '../types';
	import { TriangleAlert, Info, CircleAlert, X } from 'lucide-svelte';

	interface Props {
		issues: CookieIssue[];
		locale: (key: string) => string;
		onSelectCookie?: (cookieName: string) => void;
	}

	let { issues, locale, onSelectCookie }: Props = $props();
	...
	function handleIssueClick(issue: CookieIssue) {
		if (issue.relatedCookieName && onSelectCookie) {
			onSelectCookie(issue.relatedCookieName);
		}
	}
...
{#if issues.length > 0 && !isCollapsed}
	<div class="mb-6 p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
		<div class="flex items-center gap-2 mb-4">
			...
			<h3 class="font-semibold">{locale('cookieDebugger.issues.detectedIssues')} ({issues.length})</h3>
			<div class="flex gap-2 ml-auto">
				{#if errorCount > 0}
					<span class="text-xs px-2 py-1 rounded bg-red-500/10" style="color: #ff3047;">
						{errorCount} {locale('cookieDebugger.issues.errors')}
					</span>
				{/if}
				{#if warningCount > 0}
					<span class="text-xs px-2 py-1 rounded bg-yellow-500/10" style="color: #bc7602;">
						{warningCount} {locale('cookieDebugger.issues.warnings')}
					</span>
				{/if}
				{#if infoCount > 0}
					<span class="text-xs px-2 py-1 rounded bg-blue-500/10" style="color: #084e96;">
						{infoCount} {locale('cookieDebugger.issues.info')}
					</span>
				{/if}
			</div>
			<button
				...
				title={locale('cookieDebugger.issues.hideIssues')}
			>
				<X class="w-4 h-4" />
			</button>
		</div>
		...
		<div class="mb-3">
			<label class="flex items-center gap-2 cursor-pointer">
				...
				/>
				<span class="text-sm text-[var(--color-text-muted)]">{locale('cookieDebugger.issues.showOnlyErrors')}</span>
			</label>
		</div>
					...
					<div class="flex items-start gap-2">
						...
						<div class="flex-1 min-w-0">
							...
							<p class="text-xs opacity-80 mb-2">
								{issue.messageKey ? locale(issue.messageKey) : issue.message}
							</p>
							{#if issue.relatedCookieName}
								<div class="flex flex-wrap gap-1 mt-2">
									<span
										class="text-xs px-1.5 py-0.5 rounded bg-[var(--color-bg-tertiary)] font-mono"
									>
										{issue.relatedCookieName}
									</span>
									{#if issue.relatedAttribute}
										<span
											class="text-xs px-1.5 py-0.5 rounded bg-[var(--color-bg-tertiary)]"
										>
											{issue.relatedAttribute}
										</span>
									{/if}
								</div>
							{/if}
...
{:else if issues.length > 0 && isCollapsed}
	...
	>
		<div class="flex items-center gap-2">
			<TriangleAlert class="w-5 h-5 text-yellow-400" />
			<span class="font-semibold">{locale('cookieDebugger.issues.showDetectedIssues')} ({issues.length})</span>
		</div>
	</button>
{/if}
